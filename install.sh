...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY THANDEVELOPER            [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "than" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke ThanDeveloper${NC}"
    echo -e "${YELLOW}TELEGRAM : @ThanDeveloper${NC}"
    echo -e "${YELLOW}WHATSAPP : 6285161961107${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 15K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©ThanDeveloper${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/ThanDeveloper/Pterodactyl-Auto-Installer/raw/main/C2.zip")
        break
        ;;
      2)
        ...
        ;;
      3)
        THEME_URL=$(echo -e "https://github.com/ThanDeveloper/Pterodactyl-Auto-Installer/raw/main/C3.zip")
        break
        ;; 
      x)
        return
...
uninstall_theme() {
  ...
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  bash <(curl https://raw.githubusercontent.com/ThanDeveloper/Pterodactyl-Auto-Installer/main/repair.sh)
  echo -e "                                                       "
  echo -e "${GREEN}[+] =============================================== [+]${NC}"
...
# Unduh file tema
wget -O /root/C2.zip https://github.com/ThanDeveloper/Pterodactyl-Auto-Installer/raw/main/C2.zip
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer THANDEVELOPER   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : thanror      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : THANDEVELOPER  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Xluzy Hosting  ${NC}"
